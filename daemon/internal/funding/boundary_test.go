...
func TestFunding_SoleEntryPointDemandsAGrant(t *testing.T) {
	...
	allowed := map[string]bool{
		...
		"SettleOnChain": true,
		// The H3 payment lane (classified at the V6 review):
		// ExecutePayment DEMANDS the Grant like Execute and derives the wire intent, the
		// approvalToken and the paymentId FROM the grant: no caller-supplied payee,
		// amount or resource, so no second door. An attacker with the Agent pointer but
		// no Grant gets Execute's refusal, because the empty-grant and replay checks run
		// first, before any wire value is built (at15_test.go pins the signature AND that
		// nothing reaches the payer).
		"ExecutePayment": true,
		// Quote takes NO Grant, a deliberate exception, classified: it is a READ (an HTTP
		// 402 probe of a seller through the sidecar). It moves no money, mints no
		// authorization and touches no key. It lives here so SIDECAR_AUTH_TOKEN stays
		// confined to the one package that holds spend capabilities; the alternative was
		// handing that token to purchasing, which is the AT-15 hole this placement closes.
		"Quote": true,
		// SetPayer is a wiring-only setter, like SetChain: it installs a lane and the
		// approvalToken key, and moves nothing.
		"SetPayer": true,
	}
