...
Contradictions and gaps found while implementing against PRD v4.1 FINAL (Jul 2026).

> **Status register — restored 2026-08-08.** This file was retired from the working docs on
> 22 Jul (`8c04728`, when the v8.0 Formal PRD superseded the v4.1 working set), but fourteen
> code comments, tests, and citations still name it by path. It is restored here as a
> **resolution register**, not a live blocker list. The Jul 24 ABI freeze has passed and both
> ABI-affecting items (SPEC-01, SPEC-04) were resolved before it. Current disposition:
>
> - **SPEC-01** — ✅ RESOLVED 19 Jul: the budget term was dropped from the advance formula.
> - **SPEC-02** — ⏳ OPEN: the recording reading is implemented and tested; the releasing
>   reading is unruled. Adjacent to the unwired-`recordExpense` work (V6-F1 / issue #49) —
>   whoever wires that call chooses what a recorded expense does to the money, so record the
>   ruling before writing it.
> - **SPEC-03** — implemented (admin/operator may create; customer-only funding bounds the
>   blast radius); no formal ruling recorded.
> - **SPEC-04** — ✅ RESOLVED 19 Jul: one-shot `wireFloatPool` / `wireJobVault`.
> - **SPEC-05** — ⏳ OPEN: the flat manifest files on disk are authoritative today.
> - **SPEC-06** — ✅ RESOLVED in practice by V12: the demo pool depth is computed from the
>   advance rather than hardcoded (`scripts/README.md`), so the 0:40 beat no longer reverts.
> - **SPEC-07** — implemented as full restitution (consistent with SPEC-02's recording reading).
>
> The findings below are the original v4.1 reasoning, preserved verbatim.
...
| [SPEC-06](#spec-06) | Blocker (orig.) | Demo pool seed breaks the exposure cap | ✅ **RESOLVED (V12)** — pool depth computed, not hardcoded |
...
> ### ✅ RESOLVED — in practice by V12
> The demo pool depth is **computed from the advance** rather than hardcoded to a number that
> contradicts the cap (see `scripts/README.md`, "Why the pool depth is computed"). The seed is
> derived to keep the 0:40 "snap" advance inside the 10% per-org exposure cap, so the beat no
> longer reverts. The contract cap itself was always correct and is unchanged; this was a
> seed-number problem, ruled the same way as SPEC-01.

### Original finding: the demo pool seed cannot support the demo advance — **blocker**
