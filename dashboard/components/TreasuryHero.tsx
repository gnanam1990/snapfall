...
export default function TreasuryHero({ treasuryUsdc, orgRateBps }: { treasuryUsdc: string; orgRateBps: number }) {
    ...
    <div className="hero">
      <div>
        ...
        <div className={`hero-num${flash ? ' flash' : ''}`}>
          {formatUsdc(treasuryUsdc)} <span className="u">USDC</span>
        </div>
        <div className="stat-sub">working capital drawn against escrowed receivables</div>
      </div>
      <div className="hero-right">
        <div className="chip-arc">◆ Advance rate {formatBps(orgRateBps)}</div>
